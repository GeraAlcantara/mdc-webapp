...
function SectionTimeline({
  ...
  const renderlayout = layoutLeft ? (
    <section className={`py-4 md:py-8  ${ColorSchemaDark ? '' : 'bg-white text-gray-900'}`}>
      <div
        ref={articleRef}
        className="mdc-ui-container flex flex-col-reverse gap-4 md:gap-8 md:flex-row "
      >
        <div
        ...
        >
          ...
          <Link href={link.slug}>
            ...
            >
              <span className={` ${ColorSchemaDark ? 'text-gray-900' : 'text-brandWhite'}  `}>
                {link.text}
              </span>
            </a>
          </Link>
        </div>
        <TimelineCheckmark bgColor={colorClass} />
        <div
          className={`md:w-1/2 opacity-0 ${
            isInViewport ? 'animDelay animate-FadeInSlidein ' : ' animate-FadeOutSlideout '
          }`}
        >
          <Image alt={MainImage.alt} src={MainImage.src} />
        </div>
      </div>
    </section>
  ) : (
        ...
        <div className=" md:w-1/2 relative">
          ...
          {sideImage.src ? (
            <div className="absolute top-[7%] left-[15%] w-[25%]">
              <Image alt={sideImage.alt} src={sideImage.src} />
            </div>
          ) : null}
        </div>
        <TimelineCheckmark bgColor={colorClass} />
